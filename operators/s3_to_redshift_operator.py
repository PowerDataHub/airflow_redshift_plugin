...
class S3ToRedshiftOperator(BaseOperator):
    ...
    template_fields = ("task_id", "s3_key", "origin_schema")
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not (
            isinstance(self.foreign_key, dict) or isinstance(self.foreign_key, list)
        ):
            raise Exception(
                "Foreign Keys must be specified as either a dictionary or a list of dictionaries."
            )

        if self.distkey and (
            ("," in self.distkey) or not isinstance(self.distkey, str)
        ):
    ...
    def execute(self, context):
        ...
        logging.info("Primary key: {}".format(self.primary_key))
        logging.info("Dist Key: {}".format(self.distkey))
        logging.info("Sort Key: {}".format(self.sortkey))
        logging.info("Schema: {}".format(self.origin_schema))
    ...
    def read_and_format(self):
        if self.schema_location.lower() == "s3":
            ...
            schema = (
                hook.get_key(
                    self.origin_schema, bucket_name="{0}".format(self.s3_bucket)
                )
                .get()["Body"]
                .read()
                .decode("utf-8")
    ...
    def reconcile_schemas(self, schema, pg_hook):
        ...
        pg_query = """
            ...
            """.format(
            self.redshift_schema, self.table
        )
                ...
                for e in schema:
                    if i == e["name"]:
                        alter_query = """
                         ...
                         ADD COLUMN "{2}" {3}
                         """.format(
                            self.redshift_schema, self.table, e["name"], e["type"]
                        )
    ...
    def copy_data(self, pg_hook, schema=None):
        ...
        def getS3Conn():
            ...
            if aws_key and aws_secret:
                creds = "aws_access_key_id={0};aws_secret_access_key={1}".format(
                    aws_key, aws_secret
                )
            elif aws_role_arn:
        ...
        if isinstance(self.primary_key, list):
            where_pk = ""
            for i, item in enumerate(self.primary_key):
                where_pk += """
                    "{rs_schema}"."{rs_table}"."{item}" = "{rs_schema}"."{rs_table}{rs_suffix}"."{item}" 
                """.format(
        ...
        append_sql = """
            ...
            FILLTARGET
            """.format(
            self.redshift_schema, self.table, self.temp_suffix
        )
        ...
        base_sql = """
            ...
            {3};
            """.format(
            self.s3_bucket, self.s3_key, getS3Conn(), params
        )

        load_sql = """COPY "{0}"."{1}" {2}""".format(
            self.redshift_schema, self.table, base_sql
        )
        if self.load_type == "append":
            ...
            load_temp_sql = """COPY "{0}"."{1}{2}" {3}""".format(
                self.redshift_schema, self.table, self.temp_suffix, base_sql
            )
    ...
    def create_if_not_exists(self, schema, pg_hook, temp=False):
        ...
        if self.primary_key:
            if isinstance(self.primary_key, str):
                ...
                pk = ", primary key({0})".format(", ".join(self.primary_key))

        if self.foreign_key:
            if isinstance(self.foreign_key, list):
                fk = ", "
                for i, e in enumerate(self.foreign_key):
                    fk += 'foreign key("{0}") references {1}("{2}")'.format(
                        e["column_name"], e["reftable"], e["ref_column"]
                    )
        ...
        if self.sortkey:
            if isinstance(self.sortkey, list):
                sk += "{0} sortkey({1})".format(
                    self.sort_type, ", ".join(["{}".format(e) for e in self.sortkey])
                )
            elif isinstance(self.sortkey, str):
                sk += "{0} sortkey({1})".format(self.sort_type, self.sortkey)
